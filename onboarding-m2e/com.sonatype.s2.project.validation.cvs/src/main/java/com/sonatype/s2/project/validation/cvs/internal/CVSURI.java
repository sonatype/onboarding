...
public class CVSURI
{
    ...
    private final CVSRepositoryLocation repository;
    ...
    public static CVSURI fromUri( URI uri )
    {
        try
        {
            CVSRepositoryLocation repository = getRepository( uri );
            if ( repository != null )
            {
        ...
        catch ( CVSException e )
        {
            ...
            throw new IllegalArgumentException( NLS.bind( "Invalid uri {0}: {1}",
                                                          new String[] { uri.toString(), e.getMessage() } ) );
        }
    }
    ...
    private static CVSRepositoryLocation getRepository( URI uri )
        throws CVSException
    {
    ...
    private static CVSRepositoryLocation getOldRepository( URI uri )
        throws CVSException
    {
    ...
    public CVSURI( CVSRepositoryLocation repository, IPath path, CVSTag tag )
    {
        this( repository, path, tag, null );
    }

    public CVSURI( CVSRepositoryLocation repository, IPath path, CVSTag tag, String revision )
    {
    ...
    {
        return new CVSURI( repository, path == null ? new Path( null, name ) : path.append( name ), tag );
    }

    public CVSURI append( IPath childPath )
    {
        return new CVSURI( repository, path == null ? childPath : path.append( childPath ), tag );
    }
    ...
    public URI toURI()
    {
        try
        {
            ...
            String pathString = path == null ? "" : path.toString(); //$NON-NLS-1$
    ...
    {
        return new CVSURI( repository, path == null ? null : path.removeLastSegments( 1 ), tag );
    }
    ...
    {
        return path == null ? null : new RemoteFolder( null, repository, path.toString(), tag );
    }

    public ICVSRemoteFile toFile()
    {
        // TODO: What about keyword mode?
        return path == null ? null : RemoteFile.create( path.toString(), repository, tag, revision );
    }

    public String toString()
    {
        return "[Path: " + ( path == null ? "null" : this.path.toString() ) + " Tag: " + tag.getName() + " Repo: " + repository.getRootDirectory() + "]"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    }
