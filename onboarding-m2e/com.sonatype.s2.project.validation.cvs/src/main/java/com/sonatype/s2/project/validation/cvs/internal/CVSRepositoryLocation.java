...
/*
 * Modified from org.eclipse.team.internal.ccvs.core.connection.CVSRepositoryLocation
 * 
 * Persistence removed
 */
    ...
    implements ICVSRepositoryLocation, IUserInfo
{
    ...
    /**
     * The name of the preferences node in the CVS preferences that contains the known repositories as its children.
     */
    public static final String PREF_REPOSITORIES_NODE = "repositories"; //$NON-NLS-1$

    /*
     * The name of the node in the default scope that has the default settings for a repository.
     */
    ...
    false
                    : ( Boolean.valueOf( System.getProperty( "eclipse.cvs.standalone" ) ).booleanValue() ); //$NON-NLS-1$ 
    ...
    /*
     * Field that indicates which connection method is to be used for locations that use the EXT connection method.
     */
    private static String extProxy;

    /*
     * Field that indicates that the last connection attempt made to this repository location failed due to an
     * authentication failure. When this is set, subsequent attempts should prompt before attempting to connect
     */
    private boolean previousAuthenticationFailed = false;

    /**
     * Return a preferences node that contains suitable defaults for a repository location.
     * 
    ...
    public static Preferences getDefaultPreferences()
    {
        Preferences defaults =
            new DefaultScope().getNode( CVSProviderPlugin.ID ).node( DEFAULT_REPOSITORY_SETTINGS_NODE );
    ...
    /**
     * Set the proxy connection method that is to be used when a repository location has the ext connection method. This
     * is usefull with the extssh connection method as it can be used to kepp the sandbox compatible with the command
     * line client.
     * 
     * @param string
     */
    ...
    /**
     * Create a repository location instance from the given properties. The supported properties are: connection The
     * connection method to be used user The username for the connection (optional) password The password used for the
     * connection (optional) host The host where the repository resides port The port to connect to (optional) root The
     * server directory where the repository is located encoding The file system encoding of the server
     */
        ...
        throws CVSException
    {
        ...
        if ( method == null )
            throw new CVSException( new Status( IStatus.ERROR, CVSProviderPlugin.ID, TeamException.UNABLE,
                                                NLS.bind( CVSMessages.CVSRepositoryLocation_methods,
        ...
        if ( host == null )
            throw new CVSException( new Status( IStatus.ERROR, CVSProviderPlugin.ID, TeamException.UNABLE,
                                                CVSMessages.CVSRepositoryLocation_hostRequired, null ) );//
        ...
        if ( root == null )
            throw new CVSException( new Status( IStatus.ERROR, CVSProviderPlugin.ID, TeamException.UNABLE,
                                                CVSMessages.CVSRepositoryLocation_rootRequired, null ) );//
    ...
    /**
     * Parse a location string and return a CVSRepositoryLocation. On failure, the status of the exception will be a
     * MultiStatus that includes the original parsing error and a general status displaying the passed location and
     * proper form. This form is better for logging, etc.
     */
        ...
        throws CVSException
    {
        ...
        catch ( CVSException e )
        {
            ...
            MultiStatus error =
                new MultiStatus( CVSProviderPlugin.ID, IStatus.ERROR,
                                 NLS.bind( CVSMessages.CVSRepositoryLocation_invalidFormat,
    ...
    /**
     * Parse a location string and return a CVSRepositoryLocation. The valid format (from the cederqvist) is:
     * :method:[[user][:password]@]hostname[:[port]]/path/to/repository However, this does not work with CVS on NT so we
     * use the format :method:[user[:password]@]hostname[#port]:/path/to/repository Some differences to note: The :
     * after the host/port is not optional because of NT naming including device e.g.
     * :pserver:username:password@hostname#port:D:\cvsroot Also parse alternative format from WinCVS, which stores
     * connection parameters such as username and hostname in method options:
     * :method[;option=arg...]:other_connection_data e.g.
     * :pserver;username=anonymous;hostname=localhost:/path/to/repository If validateOnly is true, this method will
     * always throw an exception. The status of the exception indicates success or failure. The status of the exception
     * contains a specific message suitable for displaying to a user who has knowledge of the provided location string.
     * 
        ...
        try
        {
            ...
            if ( start == 0 )
            {
                ...
                if ( optionStart != -1 )
                {
                    ...
                    StringTokenizer stOpt = new StringTokenizer( location.substring( optionStart + 1, end ), "=;" //$NON-NLS-1$
                    );
            ...
            String host =
                ( hmOptions.containsKey( "hostname" ) ) ? hmOptions.get( "hostname" ).toString() : location.substring( start, hostEnd ); //$NON-NLS-1$ //$NON-NLS-2$
            ...
            return new CVSRepositoryLocation( method, user, password, host, port, root, null /* encoding */,
        ...
        if ( pluggedInConnectionMethods == null )
        {
            ...
            else
            {
                IExtension[] extensions =
                    Platform.getExtensionRegistry().getExtensionPoint( CVSProviderPlugin.ID,
                                                                       CVSProviderPlugin.PT_CONNECTIONMETHODS ).getExtensions();
                for ( int i = 0; i < extensions.length; i++ )
                {
                    ...
                    if ( configs.length == 0 )
                    {
                        CVSProviderPlugin.log( IStatus.ERROR,
                                               NLS.bind( "Connection method {0} is missing required fields", new Object[] { extension.getUniqueIdentifier() } ), null );//$NON-NLS-1$ 
                        continue;
                    }
                    try
                    {
                    ...
                    {
                        CVSProviderPlugin.log( IStatus.ERROR,
                                               NLS.bind( "Could not instantiate connection method for  {0}", new Object[] { extension.getUniqueIdentifier() } ), ex );//$NON-NLS-1$ 
                    }
                }
            }
    ...
    /*
     * Return the connection method registered for the given name or <code>null</code> if none is registered with the
     * given name.
     */
    ...
    /*
     * Return a string containing a list of all connection methods that is suitable for inclusion in an error message.
     */
    ...
    {
        IExtension[] extensions =
            Platform.getExtensionRegistry().getExtensionPoint( CVSProviderPlugin.ID, CVSProviderPlugin.PT_AUTHENTICATOR ).getExtensions();
        ...
        {
            CVSProviderPlugin.log( IStatus.ERROR,
                                   NLS.bind( "User autheticator {0} is missing required fields", ( new Object[] { extension.getUniqueIdentifier() } ) ), null );//$NON-NLS-1$ 
            return null;
        }
        try
        {
        ...
        {
            CVSProviderPlugin.log( IStatus.ERROR,
                                   NLS.bind( "Unable to instantiate user authenticator {0}", ( new Object[] { extension.getUniqueIdentifier() } ) ), ex );//$NON-NLS-1$ 
            return null;
        }
    }
     ...
     */
    private CVSRepositoryLocation( IConnectionMethod method, String user, String password, String host, int port,
                                   String root, String encoding, boolean userFixed, boolean passwordFixed )
    {
        this.method = method;
        this.user = user;
    ...
    /*
     * Create the connection to the remote server. If anything fails, an exception will be thrown and must be handled by
     * the caller.
     */
        ...
        throws CVSException
    {
        ...
        IServerConnection conn = methodToUse.createConnection( this, password );
        Connection connection = new Connection( this, conn );
        connection.open( monitor );
        getPassword( conn );
        return connection;
    }

    private void getPassword( IServerConnection conn )
    {
        // if ( conn instanceof CVSSSH2ServerConnection )
        // {
        // try
        // {
        // Field f = CVSSSH2ServerConnection.class.getDeclaredField( "session" );
        // f.setAccessible( true );
        // Object session = f.get( conn );
        // if ( session == null )
        // return;
        ...
        // if ( session == null )
        // return;
        // f = session.getClass().getDeclaredField( "password" );
        // f.setAccessible( true );
        // }
        // catch ( Exception e )
        // {
    ...
    /*
     * @see IRepositoryLocation#getLocation() The username is included if it is fixed. The password is never included
     * even if it is fixed. The port is included if it is not the default port.
     */
    ...
    public String getLocation( boolean forDisplay )
    {
        return COLON + method.getName() + COLON
            + ( userFixed ? ( user + ( ( passwordFixed && !forDisplay ) ? ( COLON + password ) : "" )//$NON-NLS-1$ 
            + HOST_SEPARATOR ) : "" ) + //$NON-NLS-1$ 
            host + COLON + ( ( port == USE_DEFAULT_PORT ) ? "" : ( new Integer( port ).toString() ) ) + //$NON-NLS-1$ 
            root;
    }
    ...
    public ICVSRemoteFile getRemoteFile( String remotePath, CVSTag tag )
    {
        ...
        RemoteFile remoteFile =
            new RemoteFile( remoteFolder, Update.STATE_ADDED_LOCAL, path.lastSegment(), null, null, tag );
    ...
    /*
     * @see ICVSRepositoryLocation#getTimeout() For the time being, the timeout value is a system wide value associated
     * with the CVSPlugin singleton.
     */
     ...
     */
    public IUserInfo getUserInfo( final boolean makeUsernameMutable )
    {
        return new IUserInfo()
        {
            ...
            private boolean mutable = makeUsernameMutable ? true : isUsernameMutable();
    ...
    /**
     * Return true if the password is mutable. If not, setPassword should not be called
     */
    ...
    /*
     * Open a connection to the repository represented by the receiver. If the username or password are not fixed,
     * openConnection will use the plugged-in authenticator to prompt for the username and/or password if one has not
     * previously been provided or if the previously supplied username and password are invalid. This method is
     * synchronized to ensure that authentication with the remote server is serialized. This is needed to avoid the
     * situation where multiple failed authentications occur and result in the remote account being locked. The
     * CVSProviderPlugin enforces that there is one instance of a CVSRepositoryLocation per remote location thus this
     * method is called for any connection made to this remote location.
     */
        ...
        throws CVSException
    {
        ...
        try
        {
            ...
            monitor.beginTask( NLS.bind( CVSMessages.CVSRepositoryLocation_openingConnection,
                                         new String[] { getHost() } ), 2 );
        ...
        throws CVSException
    {
        ...
        if ( authenticator == null )
        {
            throw new CVSAuthenticationException( CVSMessages.CVSRepositoryLocation_noAuthenticator,
                                                  CVSAuthenticationException.NO_RETRY, this );//
        }
    ...
    /*
     * Validate that the receiver contains valid information for making a connection. If the receiver contains valid
     * information, the method returns. Otherwise, an exception indicating the problem is throw.
     */
    ...
    /**
     * Return the server platform type. It will be one of the following: UNDETERMINED_PLATFORM: The platform has not
     * been determined CVS_SERVER: The platform is regular CVS server CVSNT_SERVER: The platform in CVSNT If
     * UNDETERMINED_PLATFORM is returned, the platform can be determined using the Command.VERSION command.
     */
    ...
    private String stringReplace( String string, String oldString, String newString )
    {
        ...
        if ( index == -1 )
            return string;
        return stringReplace( string.substring( 0, index ) + newString + string.substring( index + oldString.length() ),
                              oldString, newString );
    }

    /**
     * Return the server message with the prefix removed. Server aborted messages typically start with "cvs server: ..."
     * "cvs [server aborted]: ..." "cvs rtag: ..."
     */
