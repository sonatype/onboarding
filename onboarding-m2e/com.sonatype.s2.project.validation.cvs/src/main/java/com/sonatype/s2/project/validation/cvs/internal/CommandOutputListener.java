...
/*
 * Copied from org.eclipse.team.internal.ccvs.core.client.CommandOutputListener
 */
@SuppressWarnings( "restriction" )
public class CommandOutputListener
    implements ICommandOutputListener
{
    public static ICommandOutputListener INSTANCE = new CommandOutputListener();

    /*
     * Failure string that is returned from the server when pserver is used and the root directory is not readable. The
     * problem can be fixed by making the directory readable or by using -f in the pserver configuration file. We will
     * ignore the error since it does not affect the command.
     */
    public static final String ROOT_CVSIGNORE_READ_FAILURE =
        "cvs server: cannot open /root/.cvsignore: Permission denied"; //$NON-NLS-1$
                              ...
                              IProgressMonitor monitor )
    {
        ...
        if ( line.equals( ROOT_CVSIGNORE_READ_FAILURE )
            || getServerMessage( ROOT_CVSIGNORE_READ_FAILURE, location ).equals( getServerMessage( line, location ) ) )
        {
    ...
    /**
     * Return the portion of the line that describes the error if the error line is a protocol error or null if the line
