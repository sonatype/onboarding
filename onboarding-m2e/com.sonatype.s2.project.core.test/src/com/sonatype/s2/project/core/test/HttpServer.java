...
public class HttpServer
{
    ...
    private Map<String, ErrorResponse> resourceErrorResponses = new HashMap<String, ErrorResponse>();
    ...
    private static class ErrorResponse
    {
        ...
        public ErrorResponse( int httpErrorCode, String content )
        {
    ...
    protected Connector newHttpsConnector()
    {
        ...
        String keystore = new File( "resources/ssl/keystore" ).getAbsolutePath();
     ...
     * 
     * @param httpPort The port to use, may be {@code 0} to pick a random port, if negative the HTTP connector will be
     *            disabled.
    ...
    {
        resourceErrorResponses.put( resourcePath, new ErrorResponse( httpErrorCode, null ) );
     ...
     * @return This server, never {@code null}.
     */
    public HttpServer addResourceErrorResponse( String resourcePath, int httpErrorCode, String responseContent )
    {
        resourceErrorResponses.put( resourcePath, new ErrorResponse( httpErrorCode, responseContent ) );
        ...
        throws Exception
    {
        ...
        if ( !resourceDirs.isEmpty() || !resourceErrorResponses.isEmpty() )
        {
        ...
        extends AbstractHandler
    {
            ...
            throws IOException, ServletException
        {
            ...
            for ( String resourcePath : resourceErrorResponses.keySet() )
            {
                ...
                {
                    ErrorResponse errorResponse = resourceErrorResponses.get( resourcePath );
                    if ( errorResponse.content != null )
                    {
                        response.getOutputStream().write( errorResponse.content.getBytes() );
                    }
                    else
                    {
                        response.getOutputStream().write( ( "***Error" + errorResponse.httpErrorCode + "***" ).getBytes() );
                    }
                    response.setStatus( errorResponse.httpErrorCode );
                    ( (Request) request ).setHandled( true );
                    return;
                }
            }
