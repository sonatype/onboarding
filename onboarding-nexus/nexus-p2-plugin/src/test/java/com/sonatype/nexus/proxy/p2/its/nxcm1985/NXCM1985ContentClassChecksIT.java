 ...
 */
package com.sonatype.nexus.proxy.p2.its.nxcm1985;
...
/**
 * This test is basically duplicate of Nxcm1955SecureRestResourceIT, simply using the m2 target privs instead of the p2
 * lineup target privs, to make sure that the compatability of the 2 content classes works properly (m2 -> p2lineup)
 */
public class NXCM1985ContentClassChecksIT
    extends AbstractSecureP2LineupIT
{
    ...
    public NXCM1985ContentClassChecksIT()
    {
        super( "p2lineups" );
        ...
        throws Exception
    {
        ...
        // give user access to .* we have UT's that test the actual target privs, we just want to make sure the response
        // codes are correct. (and that it gets created)
        this.giveUserPrivilege( TEST_USER_NAME, "T5" ); // target priv
        this.giveUserPrivilege( TEST_USER_NAME, "T3" ); // target priv
        ...
        throws Exception
    {
        ...
        try
        {
            response = RequestFacade.doGetRequest( URI_PART + "/groupid/one/testGetAccess/version1" );
            String responseText = response.getEntity().getText();
        ...
        this.giveUserPrivilege( TEST_USER_NAME, "T1" ); // target priv
        TestContainer.getInstance().getTestContext().setUsername( TEST_USER_NAME );
        TestContainer.getInstance().getTestContext().setPassword( TEST_USER_PASSWORD );
        ...
        throws Exception
    {
        ...
        this.giveUserPrivilege( TEST_USER_NAME, "T7" ); // target priv
        TestContainer.getInstance().getTestContext().setUsername( TEST_USER_NAME );
        TestContainer.getInstance().getTestContext().setPassword( TEST_USER_PASSWORD );
